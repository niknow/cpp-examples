...
/* Polymorphism is one of the most powerful features of OOP. In short, 
   is means that a pointer that points to a derived class is 
   type-compatible with a pointer to its base class. This works
   particularly well with virtual functions. */
...
// base class can be passed by reference
...
int main(int argc, char *argv[])
{
    ...
    pb = &d;      // Now p points to d, although p was declared as pointer to base.
